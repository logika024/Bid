...
/* Assemble the final resume object: GPT writes content, profile owns the facts. */
...
async function init() {
  ...
  settings = await RT.storage.get([
    K.apiKey, K.model, K.genPrompt, K.profile, K.folderName, K.lastResult,
  ]);
  const folderHandle = await RT.fs.getFolder();

  const missing = [];
  if (!settings[K.apiKey]) missing.push("OpenAI API key");
...
async function generate() {
  ...
  // Secure write access NOW, while we still have a user gesture — the OpenAI
  ...
  setStatus("Writing your tailored resume with GPT…", "busy");
  try {
    const gen = await RT.openai.generateResume({
      apiKey: settings[K.apiKey],
      model: settings[K.model] || "gpt-4o",
